...
class ScrollExample_3():
    def __init__(self):
        ...
        for btn_cnt in range(2):
            list.add_button(lv.SYMBOL.AUDIO,"Track {:d}".format(btn_cnt))

            float_btn = lv.button(list)
            ...
            float_btn.set_style_bg_image_src(lv.SYMBOL.PLUS, 0)
    ...
    def float_btn_event_cb(self,e,list):
        ...
        if code == lv.EVENT.CLICKED:
            list_btn = list.add_button(lv.SYMBOL.AUDIO, "Track {:d}".format(self.btn_cnt))
            self.btn_cnt += 1

            float_btn.move_to_index(-1)

            list_btn.scroll_to_view(None)
